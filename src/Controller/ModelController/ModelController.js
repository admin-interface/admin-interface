 ...
 * @returns {Promise.<*>}
 */
export async function postUpdateSingleModel(req: express$Request, res: express$Response, next: express$NextFunction) {
    const modelKey: string           = req.params.model_key;
    ...
    if (Model) {
        ...
        try {
            ...
            const item = await Model.getModel().findById(itemId);
            if (!item) {
                return res.status(404).send(`Item not found by id: ${ itemId }`);
            }
...
export async function deleteSingleModel(req: express$Request, res: express$Response, next: express$NextFunction) {
    ...
    if (Model) {
        try {
            ...
            return next(new Error(`Primary Key not found by model key: ${ modelKey }`));
        } catch (err) {
            return next(err);
        }
    }
